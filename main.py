...
def calculate_moving_averages(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data['Signal'] = np.where(data['SMA_20'] > data['SMA_50'], 1, 0)  # Buy signal when SMA_20 crosses above SMA_50
    data['Signal'] = np.where(data['SMA_20'] < data['SMA_50'], -1, data['Signal'])  # Sell signal when SMA_20 crosses below SMA_50
    return data
...
def main():
    ...
    if st.sidebar.button("Analyze"):
        ...
        if ticker_data:
            ...
            # SMA Crossover Strategy and Performance Calculation
            for ticker, data in ticker_data.items():
                # Add a 'Position' column to track holding status
                data['Position'] = 0
